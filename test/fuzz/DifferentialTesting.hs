...
--
-- @since 0.7.1.0
module Test.Language.Javascript.DifferentialTesting
...
parseWithBabel input = do
  result <- timeout (5 * 1000000) $ 
    readProcessWithExitCode "node" 
      ["-e", "console.log(JSON.stringify(require('@babel/parser').parse(process.argv[1])))"]
      [Text.unpack input]
  case result of
...
parseWithTypeScript input = do
  result <- timeout (5 * 1000000) $
    readProcessWithExitCode "node"
      ["-e", "console.log(JSON.stringify(require('typescript').createSourceFile('test.js', process.argv[1], 99)))"]
      [Text.unpack input]
  case result of
    Just (ExitSuccess, output, _) -> return (Just output)
    _ -> return Nothing
