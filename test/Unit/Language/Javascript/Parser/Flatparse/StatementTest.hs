...
-- parsed by the flatparse-based parser. Tests cover:
--
...
--
-- All tests use the public parsing API rather than internal Grammar module.
...
-- | Test helper to parse a single statement via the public API.
-- Returns either an error string or the parsed JSStatement.
parseStmt :: String -> Either String JSStatement
parseStmt input =
  case Parser.parseStatement input "test" of
    Right (JSAstStatement stmt _) -> Right stmt
    Right _ -> Left "Expected JSAstStatement wrapper"
    Left err -> Left err
...
testBasicStatements = describe "expression statements" $ do
  ...
  it "parses function call statements" $ do
    case parseStmt "console.log('hello');" of
...
testFunctionDeclarations = describe "function declarations" $ do
  it "parses function without parameters" $ do
    case parseStmt "function foo() {}" of
  ...
  it "parses function with parameters" $ do
    case parseStmt "function add(a, b) { return a + b; }" of
  ...
  it "parses function with default parameters" $ do
    case parseStmt "function greet(name = 'World') {}" of
...
testClassDeclarations = describe "class declarations" $ do
  ...
  it "parses class with superclass" $ do
    case parseStmt "class Foo extends Bar {}" of
  ...
  it "parses class with constructor" $ do
    case parseStmt "class Foo { constructor() {} }" of
  ...
  it "parses class with methods" $ do
    case parseStmt "class Foo { bar() {} baz() {} }" of
