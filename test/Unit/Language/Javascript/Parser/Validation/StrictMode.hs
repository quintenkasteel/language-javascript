  ...
  describe "eval as identifier in expression contexts" $ do
    ...
    testReservedInContext "eval" "function parameter" $
      JSFunction
        ...
        noAnnot
        (JSBlock noAnnot [useStrictStmt] noAnnot)
        auto

    testReservedInContext "eval" "function name" $
      JSFunction
        noAnnot
        (JSIdentName noAnnot "eval")
        noAnnot
        (JSLNil)
        noAnnot
        (JSBlock noAnnot [useStrictStmt] noAnnot)
      ...
      JSLet
        ...
        ( JSLOne
            ( JSVarInitExpression
                ...
                (JSVarInit noAnnot (JSArrayLiteral noAnnot [] noAnnot))
            )
        )
        auto
    ...
    testReservedInContext "eval" "object property shorthand" $
      JSExpressionStatement
        ( JSObjectLiteral
            noAnnot
            (JSCTLNone (JSLOne (JSPropertyNameandValue (JSPropertyIdent noAnnot "eval") noAnnot [])))
            noAnnot
        )
        auto
  ...
  describe "arguments as identifier in expression contexts" $ do
    ...
    testReservedInContext "arguments" "function parameter" $
      JSFunction
        ...
        (JSIdentName noAnnot "test")
        noAnnot
        (JSLOne (JSIdentifier noAnnot "arguments"))
        noAnnot
        (JSBlock noAnnot [useStrictStmt] noAnnot)
        auto
  ...
  describe "reserved words in complex binding patterns" $ do
            ...
            ( JSVarInitExpression
                ( JSArrayLiteral
                    ...
                    [ JSArrayElement
                        ( JSArrayLiteral
                            noAnnot
                            [JSArrayElement (JSIdentifier noAnnot "eval")]
                            noAnnot
                        )
                    ]
                    noAnnot
                ...
                ( JSObjectLiteral
                    ...
                    ( createObjPropList
                        [ ( JSPropertyIdent noAnnot "nested",
                            [ JSObjectLiteral
                                noAnnot
                                (createObjPropList [(JSPropertyIdent noAnnot "arguments", [])])
                                noAnnot
                            ]
                          )
                        ]
    ...
    testReservedInContext "eval" "rest parameter pattern" $
      JSFunction
        ...
        (JSIdentName noAnnot "test")
        noAnnot
        (JSLOne (JSSpreadExpression noAnnot (JSIdentifier noAnnot "eval")))
        noAnnot
        (JSBlock noAnnot [useStrictStmt] noAnnot)
        auto
    ...
    it "validates eval in template literal expression" $ do
      let program =
            createStrictProgram
              [ JSExpressionStatement
                  ( JSTemplateLiteral
                      (Just (JSIdentifier noAnnot "eval"))
                      noAnnot
                      "hello"
                      [JSTemplatePart (JSIdentifier noAnnot "x") noAnnot "world"]
                  )
                  auto
              ]
      validateProgram program `shouldFailWith` isReservedWordError "eval"
            ...
            createStrictProgram
              [ JSExpressionStatement
                  ( JSTemplateLiteral
                      ...
                      noAnnot
                      "hello"
                      [JSTemplatePart (JSIdentifier noAnnot "arguments") noAnnot "world"]
                  )
                  auto
              ]
      validateProgram program `shouldFailWith` isReservedWordError "arguments"
