  ...
  describe "Task 14: Parser Error Condition Tests" $ do
    ...
    describe "malformed syntax recovery tests" $ do
      it "detects malformed template literals" $ do
        ...
        let validProgram = JSAstProgram
              [ JSExpressionStatement
                  (JSTemplateLiteral Nothing noAnnot "valid template"
                    [ JSTemplatePart (JSIdentifier noAnnot "name") noAnnot " world"
                    ])
                  auto
              ]
      ...
      it "validates async await in different contexts" $ do
        let validAsyncProgram = JSAstProgram
              [ JSAsyncFunction noAnnot noAnnot
      ...
      it "validates generator yield expressions" $ do
        let validGenProgram = JSAstProgram
              [ JSGenerator noAnnot noAnnot
    ...
    describe "error condition edge cases" $ do
      it "validates multiple private field accesses in expression" $ do
        ...
        case validate invalidProgram of
          Left errors ->
            let privateFieldErrors = filter (\err -> case err of
      ...
      it "rejects super property access outside method" $ do
        let invalidProgram = JSAstProgram
              [ JSClass noAnnot
                  ...
                  JSExtendsNone
                  noAnnot
                  [ JSPrivateField noAnnot "field" noAnnot 
                      (Just (JSMemberDot
                        (JSIdentifier noAnnot "super")
                        noAnnot
                        (JSIdentifier noAnnot "value")))
    ...
    describe "duplicate import/export validation" $ do
      ...
      it "rejects duplicate import names" $ do
        let invalidModule = JSAstModule
              ...
              , JSModuleImportDeclaration noAnnot
                  (JSImportDeclaration
                    (JSImportClauseNamed (JSImportsNamed noAnnot
              ...
              [ JSLabelled (JSIdentName noAnnot "outer") noAnnot
                  (JSForVar noAnnot noAnnot noAnnot
                    (JSLOne (JSVarInitExpression 
                      ...
                      (JSVarInit noAnnot (JSDecimal noAnnot "0"))))
                    noAnnot
                    (JSLOne (JSExpressionBinary
                      (JSIdentifier noAnnot "i")
                      (JSBinOpLt noAnnot)
                      (JSDecimal noAnnot "10")))
                    noAnnot
                    (JSLOne (JSExpressionPostfix
                      (JSIdentifier noAnnot "i")
                      (JSUnaryOpIncr noAnnot)))
                    noAnnot
                    (JSStatementBlock noAnnot
                      [ JSLabelled (JSIdentName noAnnot "inner") noAnnot
                          (JSForVar noAnnot noAnnot noAnnot
                            (JSLOne (JSVarInitExpression 
        ...
        let validProgram = JSAstProgram
              [ JSVariable noAnnot
                  (JSLOne (JSVarInitExpression
                    ...
                    (JSVarInit noAnnot (JSTemplateLiteral Nothing noAnnot "simple template"
        ...
        let validProgram = JSAstProgram
              [ JSVariable noAnnot
                  (JSLOne (JSVarInitExpression
                    ...
                    (JSVarInit noAnnot (JSTemplateLiteral Nothing noAnnot "Result: "
                      [ JSTemplatePart (JSExpressionBinary
                          (JSIdentifier noAnnot "a")
                          (JSBinOpPlus noAnnot)
                          (JSIdentifier noAnnot "b"))
                        noAnnot " done"
        ...
        let validProgram = JSAstProgram
              [ JSVariable noAnnot
                  (JSLOne (JSVarInitExpression
                    ...
                    (JSVarInit noAnnot (JSExpressionBinary
                      (JSStringLiteral noAnnot "Hello")
                      (JSBinOpPlus noAnnot)
                      (JSExpressionBinary
      ...
      it "validates getter has no parameters" $ do
        let validProgram = JSAstProgram
              [ JSClass noAnnot
                  ...
                  noAnnot
                  [ JSClassInstanceMethod 
                      (JSPropertyAccessor (JSAccessorGet noAnnot)
      ...
      it "validates setter has exactly one parameter" $ do
        let validProgram = JSAstProgram
              [ JSClass noAnnot
                  ...
                  noAnnot
                  [ JSClassInstanceMethod 
                      (JSPropertyAccessor (JSAccessorSet noAnnot)
                        (JSPropertyIdent noAnnot "value")
                        noAnnot
  ...
  describe "Task 21: Integration Tests for Complex Scenarios" $ do
    ...
    describe "multi-level nesting validation" $ do
      it "validates complex nested structures" $ do
        let complexProgram = JSAstProgram
              [ JSClass noAnnot
                                ...
                                [ JSReturn noAnnot
                                    (Just (JSCallExpression
                                      ...
                                      (JSLOne (JSArrowExpression
                                        ...
                                        noAnnot
                                        (JSConciseExpressionBody (JSExpressionBinary
                                          (JSIdentifier noAnnot "item")
