...
testCurrentUnicodeSupport = describe "Current Unicode Support" $ do
  it "handles BOM as whitespace" $ do
    testLexUnicode "var\xFEFFx"
      `shouldBe` "[VarToken,WsToken,IdentifierToken 'x']"

  it "recognizes Unicode line separators" $ do
    testLexUnicode "var x\x2028var y"
      `shouldBe` "[VarToken,WsToken,IdentifierToken 'x',WsToken,VarToken,WsToken,IdentifierToken 'y']"
    testLexUnicode "var x\x2029var y"
      `shouldBe` "[VarToken,WsToken,IdentifierToken 'x',WsToken,VarToken,WsToken,IdentifierToken 'y']"

  it "handles Unicode content in comments" $ do
    testLexUnicode "//comment\x2028var x"
      `shouldBe` "[CommentToken,WsToken,VarToken,WsToken,IdentifierToken 'x']"
    testLexUnicode "/*中文注释*/var x"
      `shouldBe` "[CommentToken,VarToken,WsToken,IdentifierToken 'x']"

  it "supports basic Unicode whitespace" $ do
    -- Test a selection of Unicode whitespace characters
    testLexUnicode "var\x00A0x"
      `shouldBe` "[VarToken,WsToken,IdentifierToken 'x']" -- Non-breaking space
    testLexUnicode "var\x2000x"
      `shouldBe` "[VarToken,WsToken,IdentifierToken 'x']" -- En quad
    testLexUnicode "var\x3000x"
      `shouldBe` "[VarToken,WsToken,IdentifierToken 'x']" -- Ideographic space
...
testUnicodePartialSupport = describe "Partial Unicode Support" $ do
  it "handles BOM at file start differently than inline" $ do
    ...
    testLexUnicode "\xFEFFvar x = 1;"
      `shouldBe` "[WsToken,VarToken,WsToken,IdentifierToken 'x',WsToken,SimpleAssignToken,WsToken,DecimalToken 1,SemiColonToken]"

  it "processes mathematical Unicode symbols as escaped" $ do
    -- Current lexer shows Unicode symbols in escaped form
    testLexUnicode "π"
      `shouldBe` "[IdentifierToken '\\u03C0']"
    testLexUnicode "Δx"
      `shouldBe` "[IdentifierToken '\\u0394x']"
  ...
  it "displays Unicode strings in escaped form" $ do
    ...
    testLexUnicode "\"中文\""
      `shouldBe` "[StringToken \\\"\\u4E2D\\u6587\\\"]"
    testLexUnicode "'Hello 世界'"
      `shouldBe` "[StringToken \\'Hello \\u4E16\\u754C\\']"
