...
--
-- @since 0.7.1.0
module Test.Language.Javascript.ErrorRecoveryBench
...
data ErrorRecoveryMetrics = ErrorRecoveryMetrics
  { baselineParseTime :: !Double        -- ^ Time for error-free parsing (ms)
  , errorRecoveryTime :: !Double        -- ^ Time for parsing with errors (ms)  
  , memoryUsage :: !Int                 -- ^ Peak memory usage during recovery
  , errorMessageTime :: !Double         -- ^ Time to generate error messages (ms)
  , recoveryOverhead :: !Double         -- ^ Overhead percentage vs baseline
  } deriving (Eq, Show)
...
testErrorCascadePerformance = describe "Error cascade handling performance" $ do
  ...
  it "handles deeply nested error contexts" $ do
    let nestedErrors = concat (replicate 10 "function f() { ") ++ "error syntax" ++
                      concat (replicate 10 " }")
...
testDeepNestingErrorRecovery = describe "Deep nesting error recovery" $ do
  ...
  it "handles deeply nested function errors" $ do
    let deepFunctions = concat (replicate 20 "function f() { ") ++ 
                       "error here" ++
                       concat (replicate 20 " }")
  ...
  it "processes nested object/array errors efficiently" $ do
    let deepNesting = concat (replicate 15 "{ a: [") ++
                     "invalid syntax" ++
                     concat (replicate 15 "] }")
