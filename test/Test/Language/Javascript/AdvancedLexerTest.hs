  ...
  Hspec.describe "Regex vs Division Disambiguation" $ do
    ...
    Hspec.describe "division operator contexts" $ do
      Hspec.it "after identifiers" $ do
        testLex "a/b" `shouldBe` 
          "[IdentifierToken 'a',DivToken,IdentifierToken 'b']"
        testLex "obj.prop/value" `shouldBe`
          "[IdentifierToken 'obj',DotToken,IdentifierToken 'prop',DivToken,IdentifierToken 'value']"
        testLex "this/that" `shouldBe`
          "[ThisToken,DivToken,IdentifierToken 'that']"
          
      Hspec.it "after literals" $ do
        testLex "42/2" `shouldBe`
          "[DecimalToken 42,DivToken,DecimalToken 2]"
        testLex "'string'/length" `shouldBe`
          "[StringToken 'string',DivToken,IdentifierToken 'length']"
        testLex "true/false" `shouldBe`
          "[TrueToken,DivToken,FalseToken]"
        testLex "null/undefined" `shouldBe`
          "[NullToken,DivToken,IdentifierToken 'undefined']"
          
      Hspec.it "after closing brackets/parens" $ do
        testLex "arr[0]/divisor" `shouldBe`
          "[IdentifierToken 'arr',LeftBracketToken,DecimalToken 0,RightBracketToken,DivToken,IdentifierToken 'divisor']"
        testLex "(x+y)/z" `shouldBe`
          "[LeftParenToken,IdentifierToken 'x',PlusToken,IdentifierToken 'y',RightParenToken,DivToken,IdentifierToken 'z']"
        testLex "obj.method()/result" `shouldBe`
    ...
    Hspec.describe "regex literal contexts" $ do
      Hspec.it "after keywords that expect expressions" $ do
        testLex "return /pattern/" `shouldBe`
          "[ReturnToken,WsToken,RegExToken /pattern/]"
        testLex "throw /error/" `shouldBe`
          "[ThrowToken,WsToken,RegExToken /error/]"
        testLex "if(/test/)" `shouldBe`
          "[IfToken,LeftParenToken,RegExToken /test/,RightParenToken]"
          
      Hspec.it "after operators" $ do
        testLex "x = /pattern/" `shouldBe`
          "[IdentifierToken 'x',WsToken,SimpleAssignToken,WsToken,RegExToken /pattern/]"
        testLex "x + /regex/" `shouldBe`
          "[IdentifierToken 'x',WsToken,PlusToken,WsToken,RegExToken /regex/]"
        testLex "x || /default/" `shouldBe`
          "[IdentifierToken 'x',WsToken,OrToken,WsToken,RegExToken /default/]"
        testLex "x && /pattern/" `shouldBe`
          "[IdentifierToken 'x',WsToken,AndToken,WsToken,RegExToken /pattern/]"
          
      Hspec.it "after opening brackets/parens" $ do
        testLex "(/regex/)" `shouldBe`
          "[LeftParenToken,RegExToken /regex/,RightParenToken]"
        testLex "[/pattern/]" `shouldBe`
          "[LeftBracketToken,RegExToken /pattern/,RightBracketToken]"
        testLex "{key: /value/}" `shouldBe`
          "[LeftCurlyToken,IdentifierToken 'key',ColonToken,WsToken,RegExToken /value/,RightCurlyToken]"
          
      Hspec.it "complex regex patterns with flags" $ do
        testLex "x = /[a-zA-Z0-9]+/g" `shouldBe`
          "[IdentifierToken 'x',WsToken,SimpleAssignToken,WsToken,RegExToken /[a-zA-Z0-9]+/g]"
        testLex "pattern = /\\d{3}-\\d{3}-\\d{4}/i" `shouldBe`
          "[IdentifierToken 'pattern',WsToken,SimpleAssignToken,WsToken,RegExToken /\\d{3}-\\d{3}-\\d{4}/i]"
        testLex "multiline = /^start.*end$/gim" `shouldBe`
          "[IdentifierToken 'multiline',WsToken,SimpleAssignToken,WsToken,RegExToken /^start.*end$/gim]"
          
    Hspec.describe "ambiguous edge cases" $ do
      Hspec.it "division assignment vs regex" $ do
        testLex "x /= 2" `shouldBe`
          "[IdentifierToken 'x',WsToken,DivideAssignToken,WsToken,DecimalToken 2]"
        testLex "x = /=/g" `shouldBe`
          "[IdentifierToken 'x',WsToken,SimpleAssignToken,WsToken,RegExToken /=/g]"
          
      Hspec.it "complex expression vs regex contexts" $ do
        testLex "arr.filter(x => x/2)" `shouldBe`
          "[IdentifierToken 'arr',DotToken,IdentifierToken 'filter',LeftParenToken,IdentifierToken 'x',WsToken,ArrowToken,WsToken,IdentifierToken 'x',DivToken,DecimalToken 2,RightParenToken]"
        testLex "arr.filter(x => /pattern/.test(x))" `shouldBe`
          "[IdentifierToken 'arr',DotToken,IdentifierToken 'filter',LeftParenToken,IdentifierToken 'x',WsToken,ArrowToken,WsToken,RegExToken /pattern/,DotToken,IdentifierToken 'test',LeftParenToken,IdentifierToken 'x',RightParenToken,RightParenToken]"
  ...
  Hspec.describe "Automatic Semicolon Insertion (ASI)" $ do
    ...
    Hspec.describe "restricted production ASI" $ do
      Hspec.it "return statement ASI" $ do
        testLexASI "return\n42" `shouldBe`
          "[ReturnToken,WsToken,AutoSemiToken,DecimalToken 42]"
        testLexASI "return  \n  value" `shouldBe`
          "[ReturnToken,WsToken,AutoSemiToken,IdentifierToken 'value']"
        testLexASI "return\r\nresult" `shouldBe`
          "[ReturnToken,WsToken,AutoSemiToken,IdentifierToken 'result']"
          
      Hspec.it "break statement ASI" $ do
        testLexASI "break\nlabel" `shouldBe`
          "[BreakToken,WsToken,AutoSemiToken,IdentifierToken 'label']"
        testLexASI "break  \n  here" `shouldBe`
          "[BreakToken,WsToken,AutoSemiToken,IdentifierToken 'here']"
        testLexASI "break\r\ntarget" `shouldBe`
          "[BreakToken,WsToken,AutoSemiToken,IdentifierToken 'target']"
          
      Hspec.it "continue statement ASI" $ do
        testLexASI "continue\nlabel" `shouldBe`
          "[ContinueToken,WsToken,AutoSemiToken,IdentifierToken 'label']"
        testLexASI "continue  \n  loop" `shouldBe`
          "[ContinueToken,WsToken,AutoSemiToken,IdentifierToken 'loop']"
        testLexASI "continue\r\nnext" `shouldBe`
          "[ContinueToken,WsToken,AutoSemiToken,IdentifierToken 'next']"
      ...
      Hspec.it "throw statement ASI (not currently implemented)" $ do
        ...
        testLexASI "throw\nerror" `shouldBe`
          "[ThrowToken,WsToken,IdentifierToken 'error']"
        testLexASI "throw  \n  value" `shouldBe`
          "[ThrowToken,WsToken,IdentifierToken 'value']"
        testLexASI "throw\r\nnew Error()" `shouldBe`
          "[ThrowToken,WsToken,NewToken,WsToken,IdentifierToken 'Error',LeftParenToken,RightParenToken]"
          
    Hspec.describe "line terminator types" $ do
      Hspec.it "Line Feed (LF) \\n" $ do
        testLexASI "return\nx" `shouldBe`
          "[ReturnToken,WsToken,AutoSemiToken,IdentifierToken 'x']"
        testLexASI "break\nloop" `shouldBe`
          "[BreakToken,WsToken,AutoSemiToken,IdentifierToken 'loop']"
          
      Hspec.it "Carriage Return (CR) \\r" $ do
        testLexASI "return\rx" `shouldBe`
          "[ReturnToken,WsToken,AutoSemiToken,IdentifierToken 'x']"
        testLexASI "continue\rloop" `shouldBe`
          "[ContinueToken,WsToken,AutoSemiToken,IdentifierToken 'loop']"
      ...
      Hspec.it "Line Separator (LS) U+2028" $ do
        testLexASI ("return\x2028x") `shouldBe`
          "[ReturnToken,WsToken,AutoSemiToken,IdentifierToken 'x']"
        testLexASI ("break\x2028label") `shouldBe`
          "[BreakToken,WsToken,AutoSemiToken,IdentifierToken 'label']"
          
      Hspec.it "Paragraph Separator (PS) U+2029" $ do
        testLexASI ("return\x2029x") `shouldBe`
          "[ReturnToken,WsToken,AutoSemiToken,IdentifierToken 'x']"
        testLexASI ("continue\x2029loop") `shouldBe`
          "[ContinueToken,WsToken,AutoSemiToken,IdentifierToken 'loop']"
          
    Hspec.describe "comment interaction with ASI" $ do
      Hspec.it "single-line comments trigger ASI" $ do
        testLexASI "return // comment\nvalue" `shouldBe`
          "[ReturnToken,WsToken,CommentToken,WsToken,AutoSemiToken,IdentifierToken 'value']"
        testLexASI "break // end of loop\nlabel" `shouldBe`
          "[BreakToken,WsToken,CommentToken,WsToken,AutoSemiToken,IdentifierToken 'label']"
        testLexASI "continue // next iteration\nloop" `shouldBe`
          "[ContinueToken,WsToken,CommentToken,WsToken,AutoSemiToken,IdentifierToken 'loop']"
          
      Hspec.it "multi-line comments with newlines trigger ASI" $ do
        testLexASI "return /* comment\nwith newline */ value" `shouldBe`
          "[ReturnToken,WsToken,CommentToken,AutoSemiToken,WsToken,IdentifierToken 'value']"
        testLexASI "break /* multi\nline\ncomment */ label" `shouldBe`
          "[BreakToken,WsToken,CommentToken,AutoSemiToken,WsToken,IdentifierToken 'label']"
          
      Hspec.it "multi-line comments without newlines do not trigger ASI" $ do
        testLexASI "return /* inline comment */ value" `shouldBe`
          "[ReturnToken,WsToken,CommentToken,WsToken,IdentifierToken 'value']"
        testLexASI "break /* no newline */ label" `shouldBe`
          "[BreakToken,WsToken,CommentToken,WsToken,IdentifierToken 'label']"
          
    Hspec.describe "non-ASI contexts" $ do
      Hspec.it "normal statements do not trigger ASI" $ do
        testLexASI "var\nx = 1" `shouldBe`
          "[VarToken,WsToken,IdentifierToken 'x',WsToken,SimpleAssignToken,WsToken,DecimalToken 1]"
        testLexASI "function\nf() {}" `shouldBe`
          "[FunctionToken,WsToken,IdentifierToken 'f',LeftParenToken,RightParenToken,WsToken,LeftCurlyToken,RightCurlyToken]"
        testLexASI "if\n(condition) {}" `shouldBe`
          "[IfToken,WsToken,LeftParenToken,IdentifierToken 'condition',RightParenToken,WsToken,LeftCurlyToken,RightCurlyToken]"
  ...
  Hspec.describe "Multi-State Lexer Transitions" $ do
    ...
    Hspec.describe "template literal state transitions" $ do
      ...
      Hspec.it "template literal edge cases" $ do
        ...
        testLex "`simple`" `shouldBe`
          "[NoSubstitutionTemplateToken `simple`]"
          
    Hspec.describe "regex/division state switching" $ do
      Hspec.it "rapid context changes" $ do
        testLex "a/b/c" `shouldBe`
          "[IdentifierToken 'a',DivToken,IdentifierToken 'b',DivToken,IdentifierToken 'c']"
        testLex "(a)/b/c" `shouldBe`
          "[LeftParenToken,IdentifierToken 'a',RightParenToken,DivToken,IdentifierToken 'b',DivToken,IdentifierToken 'c']"
        testLex "a/(b)/c" `shouldBe`
          "[IdentifierToken 'a',DivToken,LeftParenToken,IdentifierToken 'b',RightParenToken,DivToken,IdentifierToken 'c']"
          
      Hspec.it "state persistence across tokens" $ do
        testLex "if (/pattern/.test(str)) {}" `shouldBe`
          "[IfToken,WsToken,LeftParenToken,RegExToken /pattern/,DotToken,IdentifierToken 'test',LeftParenToken,IdentifierToken 'str',RightParenToken,RightParenToken,WsToken,LeftCurlyToken,RightCurlyToken]"
        testLex "result = x/y + /regex/" `shouldBe`
          "[IdentifierToken 'result',WsToken,SimpleAssignToken,WsToken,IdentifierToken 'x',DivToken,IdentifierToken 'y',WsToken,PlusToken,WsToken,RegExToken /regex/]"
    ...
    Hspec.describe "whitespace and comment state handling" $ do
      Hspec.it "preserves state across whitespace" $ do
        testLex "return  \n  /pattern/" `shouldBe`
          "[ReturnToken,WsToken,RegExToken /pattern/]"
        testLex "x  \n  /  \n  y" `shouldBe`
          "[IdentifierToken 'x',WsToken,DivToken,WsToken,IdentifierToken 'y']"
  ...
  Hspec.describe "Lexer Error Recovery" $ do
    ...
    Hspec.describe "invalid numeric literal recovery" $ do
      Hspec.it "recovers from invalid octal literals" $ do
        testLex "089abc" `shouldBe`
          "[DecimalToken 0,DecimalToken 89,IdentifierToken 'abc']"
        testLex "0999xyz" `shouldBe`
          "[DecimalToken 0,DecimalToken 999,IdentifierToken 'xyz']"
          
      Hspec.it "recovers from invalid hex literals" $ do
        testLex "0xGHI" `shouldBe`
          "[DecimalToken 0,IdentifierToken 'xGHI']"
        testLex "0Xzyz" `shouldBe`
          "[DecimalToken 0,IdentifierToken 'Xzyz']"
          
      Hspec.it "recovers from invalid binary literals" $ do
        testLex "0b234" `shouldBe`
          "[DecimalToken 0,IdentifierToken 'b234']"
        testLex "0Babc" `shouldBe`
    ...
    Hspec.describe "regex error recovery" $ do
      ...
      Hspec.it "handles regex flag recovery" $ do
        testLex "x = /valid/g + /pattern/i" `shouldBe`
          "[IdentifierToken 'x',WsToken,SimpleAssignToken,WsToken,RegExToken /valid/g,WsToken,PlusToken,WsToken,RegExToken /pattern/i]"
    ...
    Hspec.describe "unicode and encoding recovery" $ do
      ...
      Hspec.it "handles unicode in string literals" $ do
        testLex "'Hello 世界'" `shouldBe`
          "[StringToken 'Hello 世界']"
        testLex "\"Σπουδαίο 📚\"" `shouldBe`
          "[StringToken \"Σπουδαίο 📚\"]"
          
      Hspec.it "handles unicode escape sequences" $ do
        testLex "'\\u0048\\u0065\\u006C\\u006C\\u006F'" `shouldBe`
          "[StringToken '\\u0048\\u0065\\u006C\\u006C\\u006F']"
        testLex "\"\\u4E16\\u754C\"" `shouldBe`
          "[StringToken \"\\u4E16\\u754C\"]"
          
    Hspec.describe "state consistency after errors" $ do
      Hspec.it "maintains proper state after numeric errors" $ do
        testLex "089 + 123" `shouldBe`
          "[DecimalToken 0,DecimalToken 89,WsToken,PlusToken,WsToken,DecimalToken 123]"
        testLex "0xGG - 456" `shouldBe`
          "[DecimalToken 0,IdentifierToken 'xGG',WsToken,MinusToken,WsToken,DecimalToken 456]"
          
      Hspec.it "maintains regex/division state after recovery" $ do
        testLex "0xZZ/pattern/" `shouldBe`
          "[DecimalToken 0,IdentifierToken 'xZZ',DivToken,IdentifierToken 'pattern',DivToken]"
        testLex "return 0xWW + /valid/" `shouldBe`
          "[ReturnToken,WsToken,DecimalToken 0,IdentifierToken 'xWW',WsToken,PlusToken,WsToken,RegExToken /valid/]"
